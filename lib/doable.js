...
function doable(marked, mdContents) {
  try {
    return _doable(marked, mdContents);
  } catch (err) {
...
function _doable(marked, mdContents) {
  ...
  renderer.code = (src, blockAnnotation, _escaped) => {
    ...
    const annotationParts = `${blockAnnotation}`.split(" ");
    if (annotationParts.length > 2) {
      throw new Error(`Invalid code-block annotation\n\n  Supported : [lang, fname]\n  Actual    : ${JSON.stringify(annotationParts)}\n\n  \`\`\`${blockAnnotation}\n  ${src.split("\n").join("\n  ")}\n  \`\`\``)
    }
    const [lang, fname] = annotationParts;
    if (lang === 'null') {
      trace(`  skipping null block`);
      return src;
    }
